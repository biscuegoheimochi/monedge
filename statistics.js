...
function loadAssetBreakdown() {
    console.log('[Statistics] Loading asset breakdown...');
    // Ensure categories are loaded for colors
    if (!assetCategories) {
        callApi('getAssetCategories')
            .then(res => {
                if (res.status === 'success') {
                    assetCategories = res.data;
                    console.log('[Statistics] Asset Categories loaded:', assetCategories);
                }
                fetchAssetAccountsForBreakdown();
            })
            .catch(err => console.error('Asset Cat Load Error:', err));
    } else {
        fetchAssetAccountsForBreakdown();
    }
}

function fetchAssetAccountsForBreakdown() {
    callApi('getAssetAccounts')
        .then(res => {
            console.log('[Statistics] Asset Accounts response:', res);
            if (res.status === 'success') {
                renderAssetBreakdownChart(res.data);
            } else {
                console.error('Asset accounts load error:', res);
            }
        })
        .catch(err => console.error('Asset Acc Load Error:', err));
}

function renderAssetBreakdownChart(accounts) {
    console.log('[Statistics] Rendering Asset Breakdown. Raw accounts:', accounts);
    const ctx = document.getElementById('assetBreakdownChart').getContext('2d');

    // Filter Assets only (exclude Liability)
    // We need to check the category TYPE for each account.
    // account.type is the Category Name.
    const assets = accounts.filter(a => {
        const catName = a.type;
        // If assetCategories is missing, we can't be sure, but 'Liability' is often the category name for generic ones.
        // Better to check against loaded categories.
        if (assetCategories) {
            const catDef = assetCategories.find(c => c.name === catName);
            if (catDef && catDef.type === 'Liability') return false;
        }
        // Fallback: if literal type is 'Liability'
        if (catName === 'Liability') return false;
        return true;
    });

    console.log('[Statistics] Filtered Asssets:', assets);

    // Group by Category
    const totals = {};
    assets.forEach(a => {
        const cat = a.type || 'Uncategorized';
        totals[cat] = (totals[cat] || 0) + Number(a.latestBalance);
    });

    const labels = Object.keys(totals);
    const data = labels.map(l => totals[l]);

    console.log('[Statistics] Asset Breakdown Data:', { labels, data });

    // Colors
    const bgColors = labels.map(l => {
        if (assetCategories) {
            const c = assetCategories.find(cat => cat.name === l);
    ...
    const totalEl = document.getElementById('asset-breakdown-total');
    if (totalEl) {
